...
enum GrenadeTrail
{
	String:szMaterial2[PLATFORM_MAX_PATH],
	String:szWidth[16],
	String:szColor[16],
	Float:fWidth,
	iColor[4],
...
public GrenadeTrails_OnMapStart()
{
	...
	{
		g_eGrenadeTrails[i][iCacheID] = PrecacheModel2(g_eGrenadeTrails[i][szMaterial2], true);
		Downloader_AddFileToDownloadsTable(g_eGrenadeTrails[i][szMaterial2]);
	}
}
...
public GrenadeTrails_Config(&Handle:kv, itemid)
{
	...
	KvGetString(kv, "material", g_eGrenadeTrails[g_iGrenadeTrails][szMaterial2], PLATFORM_MAX_PATH);
	...
	if(FileExists(g_eGrenadeTrails[g_iGrenadeTrails][szMaterial2], true))
	{
		++g_iGrenadeTrails;
		return true;
	}
