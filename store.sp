...
{
	Preview_OnClientPostAdminCheck(client);
...
public MenuHandler_Store(Handle:menu, MenuAction:action, client, param2)
{
	if (action == MenuAction_End) {
		...
		if (menu == INVALID_HANDLE) {
			...
			char displaybuf[64];
			int styleX = 0;
			GetMenuItem(menu, param2, STRING(m_szId), styleX, displaybuf, sizeof(displaybuf));
			...
			// We are being boring
				...
				if ((g_eClients[target][iCredits] >= g_eItems[m_iId][iPrice] || g_eItems[m_iId][iPlans] > 0 && g_eClients[target][iCredits] >= Store_GetLowestPrice(m_iId)) && !Store_HasClientItem(target, m_iId) && g_eItems[m_iId][iPrice] != -1) {
					if (g_eItems[m_iId][iPlans] > 0) {
						...
						return;
					} else if (g_eCvars[g_cvarConfirmation][aCache]) {
...
public DisplayItemMenu(client, itemid)
{
	...
	if (Store_HasClientItem(target, itemid)) {
		...
		new Handle:m_hMenu = CreateMenu(specialItemMenuHandler);
		new m_iPrice = Store_GetLowestPrice(itemid);
		SetMenuExitBackButton(m_hMenu, true);
		
		char m_szId[64];
		Format(m_szId, sizeof(m_szId), "%i", itemid);
		
		InsertMenuItemEx(m_hMenu, 0, ITEMDRAW_DEFAULT, m_szId, "Preview");
		char buyOption[128];
		Format(buyOption, sizeof(buyOption), "Buy %s", g_eItems[itemid][szName]);
		InsertMenuItemEx(m_hMenu, 1, (m_iPrice <= g_eClients[target][iCredits] ? ITEMDRAW_DEFAULT:ITEMDRAW_DISABLED), m_szId, buyOption);
		
		char menuTitle[64];
		Format(menuTitle, sizeof(menuTitle), "%s - %i", g_eItems[itemid][szName], m_iPrice);
		SetMenuTitle(m_hMenu, menuTitle);
		DisplayMenu(m_hMenu, client, 0);
	}
}

public specialItemMenuHandler(Handle:menu, MenuAction:action, client, param2) {
	if (action == MenuAction_End) {
		CloseHandle(menu);
	} else if (action == MenuAction_Select) {
		new target = g_iMenuClient[client];
		...
		char displaybuf[64];
		int styleX = 0;
		GetMenuItem(menu, param2, m_szId, sizeof(m_szId), styleX, displaybuf, sizeof(displaybuf));
		int itemId = StringToInt(m_szId);
		PrintToChat(client, "%s %s", m_szId, displaybuf);
		
		if (StrEqual(displaybuf, "Preview")) {
			previewItemToPlayer(client, itemId);
		} else {
			decl String:m_szTitle[128];
			Format(STRING(m_szTitle), "%t", "Confirm_Buy", g_eItems[itemId][szName], g_eTypeHandlers[g_eItems[itemId][iHandler]][szType]);
			Store_DisplayConfirmMenu(client, m_szTitle, MenuHandler_Store, 0);
		}
	}
}
